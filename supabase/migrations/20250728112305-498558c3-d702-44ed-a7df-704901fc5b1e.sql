...
-- Insert sample activation codes for testing (only if tenants table has records)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.tenants WHERE slug = 'default') THEN
...
-- Add performance optimization indexes on existing tables (only for tables that exist)
...
DO $$
BEGIN
  -- Check if farmers table has tenant_id column
  IF EXISTS (
  ...
  -- Check if lands table has tenant_id column
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
  ...
  -- Check if crop_history table has tenant_id column
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'crop_history' 
    AND column_name = 'tenant_id'
  ) THEN
  ...
  -- Check if weather_current table has tenant_id column
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'weather_current' 
    AND column_name = 'tenant_id'
  ) THEN
...
-- Optimize user_tenants queries (this table should exist)
